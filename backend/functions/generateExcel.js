...
const generateExcel = (clinics) => {
  ...
  clinics?.map((clinic, index) => {
    ...
    ws.cell(index+2, 4).string(clinic?.name).style(style);
    ws.cell(index+2, 5).string(clinic?.contact_person?.phone_number).style(style);
    ws.cell(index+2, 6).string(clinic?.contact_person?.email).style(style);
    ws.cell(index+2, 7).string(clinic?.contact_person?.position).style(style);
    ws.cell(index+2, 8).string(moment(clinic?.register_date).format('DD/MM/YYYY')).style(style);
